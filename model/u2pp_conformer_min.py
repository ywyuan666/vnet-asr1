# -*- coding: utf-8 -*-
"""
u2pp_conformer_min.py  —— 精简版 U2++ Conformer 结构实现
=========================================================================
本文件是 U2++ 核心结构的精简实现，用于结构验证与张量形状自检；
完整训练流程基于 WeNet（见 run.ps1 / run.sh）。直接运行会执行一次
前向计算并打印各模块的张量形状与损失，便于核对结构正确性。

U2++ 关键模块：
  1) Conformer 编码器块 = 半步前馈 + 多头自注意力 + 卷积模块 + 半步前馈
  2) CTC 分支（线性层 + CTC Loss）
  3) 双向 Transformer 解码器：左→右(L2R) + 右→左(R2L)
